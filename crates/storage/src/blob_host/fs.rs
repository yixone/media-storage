...
use crate::{
    StorageResult,
    blob_host::traits::{BlobHost, BlobWriter, RemoveBlobResult, RenameBlobResult},
};
...
impl FsBlobHost {
    pub fn new(mount: impl Into<PathBuf>) -> StorageResult<Self> {
    ...
    async fn create_parents(&self, path: &Path) -> StorageResult<()> {
...
impl BlobWriter for FsBlobWriter {
    ...
    async fn finalize(mut self) -> StorageResult<()> {
